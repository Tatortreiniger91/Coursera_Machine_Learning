...
predictOneVsAll <- function(all_theta, X){
  m <- dim(X)[1]
  num_labels <- dim(all_theta)[1] # 10
  
  # You need to return the following variables correctly 
  p <- matrix(0, nrow=dim(X)[1]) # 5000 * 1
  
  # Add ones to the X data matrix
  X <- cbind(rep(1, m), X) # 5000 * 401
  
  prediction <- g(X %*% t(all_theta)) # 5000 * 401 by 401*10
  p <- apply(prediction, 1, which.max)
  return(p)
}
